...
def code_profile(code, dtc):
    """Return a single data-derived synthesis <p> for format-consistency with
    the sibling sites. It aggregates the ranked-fix data that is otherwise only
    scattered across the individual fix cards -- the overall difficulty span and
    the total aftermarket parts-cost envelope -- figures stated nowhere else on
    the page (the hero prose covers the fault mechanism, the badges cover the
    system/family, the H2 covers the fix count). Every clause is omitted when its
    source column is empty, so no constant or fabricated value is ever emitted."""
    fixes = dtc.get("fixes", [])
    if not fixes:
        return ""
    ...
    # Difficulty span across the ranked fixes (real column: difficulty_level)
    order = {"Easy DIY": 0, "Moderate DIY": 1, "Professional Required": 2}
    diffs = [f.get("difficulty_level", "").strip() for f in fixes]
    diffs = [d for d in diffs if d]
    diff_clause = ""
    if diffs:
        uniq = sorted(set(diffs), key=lambda d: order.get(d, 99))
        if len(uniq) == 1:
            diff_clause = f"are all rated {html.escape(uniq[0])}"
        else:
            diff_clause = f"span {html.escape(uniq[0])} to {html.escape(uniq[-1])} in difficulty"

    # Aggregate parts-cost envelope (real columns:
    # est_parts_cost_min_usd / est_parts_cost_max_usd)
    def _num(v):
        try:
            return float(v)
        except (TypeError, ValueError):
            return None
    mins = [n for n in (_num(f.get("est_parts_cost_min_usd")) for f in fixes) if n is not None]
    maxs = [n for n in (_num(f.get("est_parts_cost_max_usd")) for f in fixes) if n is not None]
    cost_clause = ""
    if mins and maxs:
        cost_clause = (f"carry an aggregate aftermarket parts envelope of roughly "
                       f"${min(mins):.0f}–${max(maxs):.0f} USD")

    clauses = [c for c in (diff_clause, cost_clause) if c]
    if not clauses:
        return ""
    sentence = f"Documented repairs for Code {code_esc} {' and '.join(clauses)}."
    return f'<p class="code-profile">{sentence}</p>'
...
<style>
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .code-profile {{ font-size: 0.95rem; color: #c9cfd6; margin: 20px 0 0; line-height: 1.6; }}
...
<div class="wrap">
  ...
  <div class="cta-footer">
    ...
    <p style="margin-top: 14px; font-family: var(--mono); font-size: 0.8rem;">$49 SAE-only · $149 OEM Complete (6,637 manufacturer codes across 32 makes) · one-time · <a href="/#licensing">compare the two tiers</a></p>
  </div>
</div>
